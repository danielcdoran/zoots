...
export class HelloCommand extends CommandRunner {
  ...
  async run(): Promise<void> {
    ...
    let monkey = new Monkey("Sammy the Python", { tag: "Alive", health: 100 });
    let giraffe: Animal = new Giraffe("Tommy the Palomino", { tag: "Alive", health: 100 });
    let elephant: Animal = new Elephant("Elephant", { tag: "Alive", health: 100 });
    var objectArray: Array<Animal> = [monkey, giraffe, elephant]
    var i: number
    var feedArray: Array<monkeyHealthIncrease> = [createRandom(), createRandom(), createRandom()]
    for (let j = 0; j < 20; j++) {
